    ...
    private final GestureDetector.SimpleOnGestureListener mGestureListener = new GestureDetector.SimpleOnGestureListener() {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            switch (mCurrentFlingDirection) {
                case LEFT:
                case RIGHT:
                    mScroller.fling((int) mCurrentOrigin.x, (int) mCurrentOrigin.y, (int) (velocityX * mXScrollingSpeed), 0, Integer.MIN_VALUE, Integer.MAX_VALUE, (int) -(mHourHeight * 24 + mHeaderHeight + mHeaderRowPadding * 3 + mHeaderMarginBottom + mTimeTextHeight / 2 - getHeight()), 0);
                    break;
                case VERTICAL:
            ...
            for (int dayNumber = 0;
                ...
                for (int i = 0; i < mEventRects.size(); i++) {
                    if (isSameDay(mEventRects.get(i).event.getStartTime(), day) && mEventRects.get(i).event.isAllDay()) {
                        if (mEventRects.get(i).noofevent>noofevent)noofevent=mEventRects.get(i).noofevent;
...
//       float bottom = mHeaderHeight + mHeaderRowPadding * 3;
        ...
        if (mNumberOfVisibleDays!=1)canvas.drawLine(mHeaderColumnWidth,mHeaderHeight+mHeaderRowPadding*3-105 ,mHeaderColumnWidth,getHeight(),mHourSeparatorPaint);
        mHourSeparatorPaint.setStrokeWidth(mHourSeparatorHeight);
        ...
        for (int dayNumber=leftDaysWithGaps+1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            else canvas.drawLine(startPixel,mHeaderRowPadding/3.0f ,startPixel,getHeight(),mHourSeparatorPaint);
